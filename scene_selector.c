...
void	refresh_selector_buttons_1(t_rtv1 *rtv1, int i, char *str)
{
	int		id;
	char	*tmp;
	...
	if (id == 9999 && rtv1->scene.objs[i].type != triangle)     
		libui_newbutton((t_but_constr){20, 20 + 35 * i, str, str,
			0x0000ff55}, rtv1->selector_buttons, &(rtv1->c_selector_buttons));
	if (rtv1->scene.objs[i].type == triangle)
		id = rtv1->scene.objs[i].id;
	free(tmp);
}

void	refresh_selector_buttons_2(t_rtv1 *rtv1, int i, char *str)
{
	char	*tmp;
	...
	if (rtv1->scene.lights[i].type == point)
		...
		ft_strcat(str, " amb");
	libui_newbutton((t_but_constr){150, 20 + 35 * i, str, str, 0x0000ff55},
		rtv1->selector_buttons, &(rtv1->c_selector_buttons));
	free(tmp);
}
...
void	refresh_selector_buttons(t_rtv1 *rtv1)
{
	...
	i = 0;
	SDL_FillRect(rtv1->selector_surface, &((SDL_Rect){0, 0, 300, 1000}),
		0x00FFFF00);
	destroy_buttons(rtv1);
	while (i < rtv1->scene.c_objs)
	{
		refresh_selector_buttons_1(rtv1, i, str);
		i++;
	}
	i = 0;
	while (i < rtv1->scene.c_lights)
	{
		refresh_selector_buttons_2(rtv1, i, str);
		i++;
	}
}
...
{
	rtv1->selector_surface = SDL_CreateRGBSurface(0, 300, 1000, 32,
		0, 0, 0, 0);
	rtv1->selector_window = SDL_CreateWindow("Selector", 100, 200,
		300, 1000, SDL_WINDOW_SHOWN);
	rtv1->selector_renderer = SDL_CreateRenderer(rtv1->selector_window,
