...
void			libui_createedit_1(t_libui_edit *edit, SDL_Surface *text)
{
	edit->with_text = SDL_CreateRGBSurface(0, 100, 30, 32,
		0, 0, 0, 0);
	edit->with_text_active = SDL_CreateRGBSurface(0, 100, 30, 32,
		0, 0, 0, 0);
...
t_libui_edit	libui_createedit(t_edit_constr input)
{
	...
	edit.is_active = 0;
	edit = (t_libui_edit){.x = input.x, .y = input.y};
	...
	text = TTF_RenderText_Blended(font, input.text,
		(SDL_Color){0, 0, 0, 0});
	TTF_CloseFont(font);
	TTF_Quit();
	libui_createedit_1(&edit, text);
	return (edit);
}
...
void			libui_editrefresh(t_libui_edit *edits, int number)
{
	...
	edits[number].with_text = SDL_CreateRGBSurface(0, 100, 30, 32,
		0, 0, 0, 0);
	edits[number].with_text_active = SDL_CreateRGBSurface(0, 100, 30, 32,
		0, 0, 0, 0);
