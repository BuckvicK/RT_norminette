...
int		mouse_norm_1(t_rtv1 *rtv1, int x, int y, char *func)
{
...
int		mouse_norm(int x, int y, t_rtv1 *rtv1, int pressed_button, char *func)
{
	t_list_item_addr	ret;

	if (pressed_button == -1)
	{
		mouse_norm_1(rtv1, x, y, func);
	}
	else
	{
		rtv1->active_edit = -1;
...
int		mouse_pressed_left(int x, int y, t_rtv1 *rtv1)
{
	int					pressed_button;
	char				func[64];
	t_list_item_addr	ret;
	...
	else
	{
		...
		if (mouse_norm(x, y, rtv1, pressed_button, func) == -1)
			return (0);
